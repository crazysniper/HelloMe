...
const NSTimeInterval kHMRevealSidebarDefaultAnimationDuration = 0.25;
const CGFloat kHMRevealSidebarWidth = 260.0f;
const CGFloat kHMRevealSidebarFlickVelocity = 1000.0f;


#pragma mark -
#pragma mark Private Interface
@interface HMRevealViewController ()
...
@implementation HMRevealViewController
...
- (void)dragContentView:(UIPanGestureRecognizer *)panGesture {
	...
	if (panGesture.state == UIGestureRecognizerStateChanged) {
		if (sidebarShowing) {
			if (translation > 0.0f) {
				_contentView.frame = CGRectOffset(_contentView.bounds, kHMRevealSidebarWidth, 0.0f);
				self.sidebarShowing = YES;
			} else if (translation < -kHMRevealSidebarWidth) {
				_contentView.frame = _contentView.bounds;
				self.sidebarShowing = NO;
			} else {
				_contentView.frame = CGRectOffset(_contentView.bounds, (kHMRevealSidebarWidth + translation), 0.0f);
			}
		} else {
			if (translation < 0.0f) {
				_contentView.frame = _contentView.bounds;
		...
		BOOL show = (fabs(velocity) > kHMRevealSidebarFlickVelocity)
        ? (velocity > 0)
        : (translation > (kHMRevealSidebarWidth / 2));
		[self toggleSidebar:show duration:kHMRevealSidebarDefaultAnimationDuration];
...
//		} else {
//			_sidebarView.frame = CGRectMake(0.0f, 0.0f, kHMRevealSidebarWidth, CGRectGetHeight(self.view.bounds));
...
- (void)hideSidebar {
	[self toggleSidebar:NO duration:kHMRevealSidebarDefaultAnimationDuration];
}
